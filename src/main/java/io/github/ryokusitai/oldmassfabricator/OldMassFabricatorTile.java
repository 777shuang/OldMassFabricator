...
public class OldMassFabricatorTile extends TileEntity implements IInventory, ISidedInventory, IEnergySink, IWrenchable {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            // clientとの同期
            if (numUsing > 0) {
                PacketHandlerOMF.INSTANCE
                    .sendToAll(new OMFSyncPKT(this.energy, this.scrap, this.xCoord, this.yCoord, this.zCoord));
            }
        }
    }
    ...
    public void checkCreateMatter() {
        ...
        if ((stack == null ? true : stack.stackSize < this.getInventoryStackLimit())
            && this.energy >= this.needEnergy) {

            if (stack != null && stack.getItem()
                .equals(OldMassFabricator.solidMatter)) {
    ...
    public ItemStack decrStackSize(int slot, int decSize) {
        ...
        if (stack != null) {
            if (stack.stackSize <= decSize) {
                ...
                if (this.OMFItemStacks[slot].stackSize == 0) this.OMFItemStacks[slot] = null;
            }
        }
        return stack;
    }
